...
def fetch_country(code, cc, filters, ssl_ctx):
    fstr = "&".join(f"{k}={v}" for k, v in filters.items())
    url = f"https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/{code}?format=JSON&lang=EN&geo={cc}&{fstr}"
    ...
    try:
        ...
        if e.code != 400:
            print(f"  {cc}: HTTP {e.code}", file=sys.stderr)
        return None
    except Exception as e:
        print(f"  {cc}: {e}", file=sys.stderr)
        return None

    vals = data.get("value", {})
    time_idx = data.get("dimension", {}).get("time", {}).get("category", {}).get("index", {})
    if not time_idx or not vals:
        return None

    yr = sorted(time_idx.keys(), reverse=True)[0]
    y_i = time_idx[yr]
    flat_k = str(y_i)
    if flat_k in vals and vals[flat_k] is not None:
        return {"value": round(float(vals[flat_k]), 2), "year": int(yr)}
    return None

def fetch_all(ds, ssl_ctx):
    print(f"  Fetching {ds['eurostat_code']} — {len(EU27)} countries:", flush=True)
    result = {}
    for cc in EU27:
        entry = fetch_country(ds["eurostat_code"], cc, ds["dim_filters"], ssl_ctx)
        if entry:
            result[cc] = entry
            print(f"    {cc}: {entry['value']}% ({entry['year']})", flush=True)
        else:
            print(f"    {cc}: —", flush=True)
        time.sleep(0.15)
    print(f"  Total: {len(result)} values", flush=True)
    return result
...
def main():
    ssl_ctx = get_ctx()
    print("Aegis — Eurostat seed generator v4\n", flush=True)
    all_data = []
    for ds in DATASETS:
        data = fetch_all(ds, ssl_ctx)
